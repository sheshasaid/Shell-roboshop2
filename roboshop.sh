...
do
    ...
    if [ $instance != "frontend" ]; then
        IP=$(aws ec2 describe-instances --instance-ids i-0fce07e0f753c5e4d --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text)
        RECORD_NAME="$instance.$DOMAIN_NAME"
    else
        IP=$(aws ec2 describe-instances --instance-ids i-0fce07e0f753c5e4d --query 'Reservations[0].Instances[0].PublicIpAddress' --output text)
